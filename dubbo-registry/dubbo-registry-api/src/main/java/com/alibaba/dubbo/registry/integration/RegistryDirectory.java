...
/**
 * RegistryDirectory
 * 是一种动态服务目录， 实现了 NitifyListener 接口。 当注册中心服务配置发生变化后， RegistryDirectory 可收到与当前服务相关的变化
 * 收到变更通知后， RegistryDiretory 可根据配置变更信息刷新 Invoker 列表。
 * RegistryDirectory 中有几个比较重要的逻辑，
 * 第一是 Invoker 的列举逻辑，
 * 第二是接收服务配置变更的逻辑，
 * 第三是 Invoker 列表的刷新逻辑。
 *
 * 实现了 NotifyListener 接口， 通过这个接口获取注册中心变更通知
 */
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    private void refreshInvoker(List<URL> invokerUrls) {
        // invokerUrls 仅有一个元素， 且url 协议头为 empty ， 此时标识禁用所有服务
    ...
    private Map<String, Invoker<T>> toInvokers(List<URL> urls) {
        ...
        for (URL providerUrl : urls) {
            ...
            // 通过SPI 判断服务端协议是否被 消费端支持， 不支持则抛出异常。
    ...
    private void destroyUnusedInvokers(Map<String, Invoker<T>> oldUrlInvokerMap, Map<String, Invoker<T>> newUrlInvokerMap) {
        ...
        if (oldUrlInvokerMap != null) {
            ...
            for (Map.Entry<String, Invoker<T>> entry : oldUrlInvokerMap.entrySet()) {
                ...
                if (!newInvokers.contains(entry.getValue())) {
                    ...
                    // 若不包含， 则将老得 Invoker 对应的url 存入 deleted 列表中
        ...
        if (deleted != null) {
            // 遍历 deleted 集合 ， 并到老的 <url, Invoekr> 映射关系表查出 Invoker ， 销毁
    ...
    public List<Invoker<T>> doList(Invocation invocation) {
        ...
        if (localMethodInvokerMap != null && localMethodInvokerMap.size() > 0) {
            ...
            // 检测参数列表的 第一个参数是否为 String 或者 enum 类型
