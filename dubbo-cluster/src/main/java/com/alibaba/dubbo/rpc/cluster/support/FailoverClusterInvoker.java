...
public class FailoverClusterInvoker<T> extends AbstractClusterInvoker<T> {
    ...
    /**
     * FailoverClusterInvoker 的 doInvoke 方法首先是获取重试次数， 然后根据重试次数进行循环调用， 失败后进行重试。 在for 循环内，
     * 首先是通过 负载均衡组件选择一个 invoker ， 然后再通过这个 invoker 的 invoke 方法进行远程调用。 如果失败了， 记录下异常， 并进行重试。
     * 重试时会再次调用父类的 list 方法 列举 invoker。
     *
     * @param invocation
     * @param invokers
     * @param loadbalance
    ...
    public Result doInvoke(Invocation invocation, final List<Invoker<T>> invokers, LoadBalance loadbalance) throws RpcException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (i > 0) {
                ...
                // 在进行重试之前重新列举 Invoker, 这样做的好处是 ， 如果某个服务挂了， 通过调用list 可以获得最新可用的 Invoker 列表
