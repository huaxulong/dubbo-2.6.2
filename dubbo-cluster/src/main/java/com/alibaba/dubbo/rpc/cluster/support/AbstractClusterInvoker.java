...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        boolean sticky = invokers.get(0).getUrl().getMethodParameter(methodName, Constants.CLUSTER_STICKY_KEY, Constants.DEFAULT_CLUSTER_STICKY);
        {
            ...
            // 检测 invokers 列表是否包含 stickyInvoker，如果不包含， 说明 stickyInvoker 代表的服务挂了， 此时需要将其置空
            if (stickyInvoker != null && !invokers.contains(stickyInvoker)) {
                stickyInvoker = null;
            }
            //ignore concurrency problem
            // 在 sticky 为 true，且 stickyInvoker != null 的情况下。如果 selected 包含 stickyInvoker
            // 表明 stickyInvoker 对应的服务提供者可能因网络原因未能成功提供服务。
            // 但是该提供者并没挂，此时 invokers 列表中仍存在该服务提供者对应的 Invoker。
            if (sticky && stickyInvoker != null && (selected == null || !selected.contains(stickyInvoker))) {
                // availablecheck 表示开启了可用性检查， 如果开启了，则调用 stickyInvoker.isAvailable() 方法进行检查，如果检查通过了，则直接返回 stickyInvoker
    ...
    private Invoker<T> reselect(LoadBalance loadbalance, Invocation invocation,
        ...
        // 如果 availablecheck = true 表示需要检查 Invoker的可用性
    ...
    protected List<Invoker<T>> list(Invocation invocation) throws RpcException {
        // 调用 Directory 的list 方法列举 Invoker
